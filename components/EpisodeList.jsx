...
export function EpisodeList({
  ...
  useEffect(() => {
    if (!episodes) return;
    // Find where currentEpisodeIndex is in the displayEpisodes array
  ...
  }, [currentEpisodeIndex, displayEpisodes]);

  const totalPages = Math.ceil(displayEpisodes.length / pageSize);
  const currentRangeEpisodes = displayEpisodes.slice(
    page * pageSize,
    (page + 1) * pageSize
  );
      ...
      <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-white/50 dark:bg-slate-800/50 shrink-0">
        ...
        <div className="flex items-center gap-2">
           ...
           >
             ...
             <span className={`material-symbols-outlined text-[14px] transition-transform ${isReversed ? "rotate-180" : ""}`}>
               swap_vert
             </span>
           </button>
        </div>
              ...
              return (
                <div key={originalIndex} className="relative group/episode">
                  ...
                  >
                    {displayIndex}
                    {isCurrent && (
                      <span className="absolute top-0 right-0 -mt-1 -mr-1 w-2 h-2 bg-red-500 rounded-full border border-white dark:border-slate-800"></span>
                    )}
                  </button>
                  {/* Hover Tooltip */}
